...
module Bot
  ...
  module Inventory
    ...
    def inventory_embed
      ...
      if @inventory.count.zero?
        embed.description += 'No items'
      else
        @inventory.each_with_index do |val, index|
          unless val.nil? || val.zero?
            embed.description += "**#{ITEMS[index]['name']}:** #{val}\n"
          end
        end
      end
  ...
  module Anger
    ...
    def add_anger(amount = 1)
      ...
      become_angry if anger_level > 100
    end
  ...
  class Player
    ...
    def info_embed
      embed = Discordrb::Webhooks::Embed.new
      username = BOT.parse_mention("<@#{@id}>").name
      avatar = BOT.parse_mention("<@#{@id}>").avatar_url
  ...
  class Monster
    ...
    def initialize(data, channel_id)
    ...
    def info_embed
      ...
      embed.title = @name
      embed.thumbnail = {
        url: "http://monsterhunteronline.in/monsters/images/#{@icon}.png"
      }
      embed.color = @color
      embed.description = "Angry: #{angry? ? 'Yes' : 'No'}\n" \
                          "In Trap: #{trap_expired? ? 'No' : 'Yes'}"
      embed.timestamp = Time.now
      embed
    end
